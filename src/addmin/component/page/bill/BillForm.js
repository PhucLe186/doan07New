...
const BillForm = () => {
    ...
    useEffect(() => {
        const newTotal = bill.items.reduce((sum, item) => sum + item.price * item.quantity, 0);
    ...
    const handleAddItem = () => {
        ...
        setBill({
            ...bill,
            items: [...bill.items, { ...newItem, price: parseFloat(newItem.price), quantity: parseInt(newItem.quantity) }],
        });
        ...
        <div className={styles.container}>
            ...
            <form className={styles.form} onSubmit={handleSubmit}>
                ...
                <div className={styles.itemInput}>
                    <input type="text" name="name" placeholder="Tên món" value={newItem.name} onChange={handleItemChange} required />
                    <input type="number" name="quantity" placeholder="Số lượng" min="1" value={newItem.quantity} onChange={handleItemChange} required />
                    <input type="number" name="price" placeholder="Giá" value={newItem.price} onChange={handleItemChange} required />
                    <button type="button" onClick={handleAddItem}>➕ Thêm</button>
                </div>
                ...
                {bill.items.length > 0 && (
                    <ul className={styles.itemList}>
                        {bill.items.map((item, index) => (
                            <li key={index}>
                                {item.name} - SL: {item.quantity} - Giá: {item.price.toLocaleString()} VND
